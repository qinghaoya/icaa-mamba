...
class UnPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride=16, in_chans=3, embed_dim=768, norm_layer=None,
        ...
        self.proj = nn.ConvTranspose2d(embed_dim, in_chans, kernel_size=patch_size, stride=stride)
        self.norm = norm_layer(in_chans) if norm_layer else nn.Identity()

    def forward(self, x):
        B, N, C = x.shape
        assert N == self.num_patches, \
            f"Input number of patches ({N}) doesn't match model ({self.num_patches})."
        if self.unflatten:
            x = x.transpose(1, 2).reshape(B, -1, self.grid_size[0], self.grid_size[1])  # BNC -> BCHW
        x = self.proj(x)
        x = self.norm(x)
        return x
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False,
        ...
        if not self.if_bidirectional:
            ...
            for layer in self.layers:
                ...
                count += 1
                if count % 4 == 0  :  # 每4个块取一次特征
                    selected_column =hidden_states[:, 1:, :].permute(0,2,1)
        ...
        if self.if_cls_token:
            if self.use_double_cls_token:
                ...
                if self.use_middle_cls_token:
                    selected_columns.append(hidden_states[:, token_position, :])
...
class ICAAmamba(nn.Module):
    def __init__(self, depth=12,
                 ...
                 init_layer_scale=None,
                 use_double_cls_token=False,
                 use_middle_cls_token=False, **kwargs):
        factory_kwargs = {"device": device, "dtype": dtype}
        # add factory_kwargs into kwargs
        kwargs.update(factory_kwargs)
        super().__init__()

        self.visionMamba_384 = VisionMamba(
        ...
        self.adapter.add_module("linear1", nn.Linear(384,256))
        self.adapter.add_module("act2", nn.GELU())
        self.adapter.add_module("dropout3", nn.Dropout(p=0.1))
        self.adapter.add_module("linear3", nn.Linear(256, 1))
        self.sg = nn.Sigmoid()
        self.gp = nn.AdaptiveAvgPool2d(1)
        self.wffm = WFFM(in_channels=384, num_encoding_strategies=6)
        self.norm=nn.LayerNorm(384)

    def forward(self, x):
        feat= self.visionMamba_384(x)
        m = self.selective_module(feat)
        m = self.gp(m).flatten(1)
        k = self.adapter(m)
        k = self.sg(k)
        return k
@register_model
def vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        ...
        final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2",
        if_cls_token=False, if_devide_out=True, use_middle_cls_token=False, **kwargs)

...
def get_args_parser():
    ...
    parser.add_argument('--model',
                        default='vim_small_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2',
    ...
    # * Finetuning params
    parser.add_argument('--finetune', default='/media/xqh/vim_s_midclstok_ft_81p6acc.pth', help='finetune from checkpoint')
    parser.add_argument('--attn-only', action='store_true')

    # Dataset parameters
    parser.add_argument('--data-path', default='/media/xqh/data/TestImage', type=str,
                        help='dataset path')
    parser.add_argument('--train-patch-num', default=1, type=int,
                        help='dataset path')
    parser.add_argument('--data-set', default='ICAA17K', choices=['CIFAR', 'IMNET', 'INAT', 'INAT19'],
...
def main(args):
    ...
    if args.finetune:
        ...
        state_dict = model.visionMamba_384.state_dict()
        ...
        num_patches = model.visionMamba_384.patch_embed.num_patches
        num_extra_tokens = model.visionMamba_384.pos_embed.shape[-2] - num_patches
        ...
        model.visionMamba_384.load_state_dict(checkpoint_model, strict=False)

    model_ema = None
    # print(args.model_ema)
    if args.model_ema:
    ...
    input = input.to(torch.device('cuda'), dtype=torch.float)
    ...
    if args.resume:
        ...
        new_state_dict = {key: value for key, value in checkpoint.items() if key.startswith('visionMamba_384.')}
    ...
    writer = SummaryWriter(log_dir='runs/kk36')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(writer,
        ...
        for name, param in model.named_parameters():
            if torch.equal(param, initial_params[name]):
                print(f'Gradient has not been applied for parameter {name}')
        test_stats = evaluate(writer, data_loader_val, model, device, amp_autocast, epoch)
